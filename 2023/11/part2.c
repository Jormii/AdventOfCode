...
int main()
{
    ...
    expanded_row[0] = EXPANSION * empty_row[0];
    for (int i = 1; i < HEIGHT; ++i)
    {
        expanded_row[i] = 1 + expanded_row[i - 1] + (EXPANSION - 1) * empty_row[i];
    }

    int expanded_column[WIDTH];
    expanded_column[0] = EXPANSION * empty_column[0];
    for (int i = 1; i < WIDTH; ++i)
    {
        expanded_column[i] = 1 + expanded_column[i - 1] + (EXPANSION - 1) * empty_column[i];
    }
