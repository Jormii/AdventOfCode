...
def dijkstra_search(heat_map: np.ndarray) -> int:
    ...
    while len(queue) != 0:
        ...
        for (vx, vy) in V:
            ...
            row = node.row + STRAIGHT_LINE_MIN*vy
            column = node.column + STRAIGHT_LINE_MIN*vx
            ...
            column = node.column
            acc_cost = node.cost
            for i in range(STRAIGHT_LINE_MIN - 1):
                row += vy
                column += vx
                acc_cost += heat_map[row, column]

            for i in range(STRAIGHT_LINE_MIN, STRAIGHT_LINE_LIMIT + 1):
                row += vy
                column += vx
                if not (row >= 0 and row < height and column >= 0 and column < width):
                    break
